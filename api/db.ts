...
export async function getDb() {
  if (isPostgres) {
    ...
    if (!sqliteDb) {
      sqliteDb = new Database('submissions.db');
      sqliteDb.exec(`
        CREATE TABLE IF NOT EXISTS contact_submissions (
    ...
    return {
      ...
      async getSubmissions() {
        const rawContacts = sqliteDb.prepare('SELECT * FROM contact_submissions ORDER BY id DESC').all();
        const rawApplications = sqliteDb.prepare('SELECT * FROM applications ORDER BY id DESC').all();

        // Standardize keys so they are uniform (both camelCase and snake_case)
        const contacts = rawContacts.map((row: any) => ({
          id: row.id,
          firstName: row.first_name,
          first_name: row.first_name,
          lastName: row.last_name,
        ...
        const applications = rawApplications.map((row: any) => ({
          id: row.id,
          firstName: row.first_name,
          first_name: row.first_name,
          lastName: row.last_name,
